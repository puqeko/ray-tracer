...
#include <GLUT/glut.h>
#else
// This must be on the lab computer
...
void drawFloor()
{
	...
	for(int x = -400; x <= 400; x += 20)
	{
		for(int z = -400; z <= 400; z += 20)
		{
			...
			t /= 300;
			glVertex3f(x, sin(x + t) * 10, z);
			glVertex3f(x, sin(x + t) * 10, z+20);
			glVertex3f(x+20, sin(x + t + 20) * 10, z+20);
			glVertex3f(x+20, sin(x + t + 20) * 10, z);
			flag = !flag;
		}
	}
	glEnd();
...
void drawBracket()
{
	for (int i = 0; i < 2; i++) { 
		glPushMatrix();
		glTranslatef(-10, 5, i ? 17 : -17);
		glScalef(80, 10, 6);
		glutSolidCube(1.0);
		glPopMatrix();

		glPushMatrix();
		glTranslatef(-20, 25, i ? 17 : -17);
		glScalef(40, 30, 6);
		glutSolidCube(1.0);
		glPopMatrix();
	}
...
float xPos = 38.88f, yPos = 64.f;
float xVel = 241.4f, yVel = 200.f;
...
void display()  
{
	...
	glLoadIdentity();
	gluLookAt(0, cam_hgt, 200, xPos / 2, yPos / 2, 0, 0, 1, 0);
...
void update(float deltaTime)
{
	...
	if (i > 100) {
		...
		if (xVel < 0.1) {
			xVel = 241.4f; yVel = 200.f;
			xPos = 38.88f; yPos = 64.f;
			i = 0;
		}
	} else {
		i++;
...
void idle()
{
	...
	unsigned long long currentTime = glutGet(GLUT_ELAPSED_TIME);
